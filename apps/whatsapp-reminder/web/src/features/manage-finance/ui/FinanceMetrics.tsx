...
import { FINANCE_COPY } from '@/shared/config/copy';
import { 
  TrendingUp, 
  TrendingDown, 
  Zap, 
 ...
 */
export const FinanceMetrics: React.FC<{ data: FinanceSummaryDTO | null; isLoading: boolean }> = ({ data, isLoading }) => {
  if (isLoading) {
    return (
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      <MetricCard 
        ...
        value={`Rp ${formatIDR(data?.totalIncome)}`} 
      ...
      <MetricCard 
        ...
        value={`Rp ${formatIDR(data?.balance || 0)}`} 
...
export function MetricCard({ title, value, subValue, type, icon, delay = 0 }: MetricProps) {
  return (
    <motion.div 
        ...
        <Card className="shadow-subtle border border-border/80 p-6 md:p-8 space-y-6 hover:border-accent/40 hover:shadow-modern transition-all duration-500 cursor-default bg-white rounded-[2rem] group relative overflow-hidden">
            ...
            <div className="flex items-center justify-between relative z-10">
                <div className="space-y-1">
                    <span className="font-bold tracking-[0.2em] text-[8px] text-muted-foreground/30 uppercase leading-none">
                        {title}
                    </span>
            ...
            <div className="space-y-2 relative z-10">
                <h3 className="text-3xl md:text-4xl font-bold tracking-tighter text-foreground tabular-nums group-hover:text-accent transition-colors duration-500">
                {value}
                </h3>
