...
import { useChatThreads, useThreadHistory, useMarkAsRead } from '../../../hooks/useChatHooks';
import { useWebSocket, LiveEvent, WSPayload } from '../../../hooks/useWebSocket';
...
export default function AdminChatHub() {
        ...
        if (payload.event === LiveEvent.CHAT_UPDATE) {
            ...
            queryClient.setQueriesData({ queryKey: ['chats'] }, (oldData: PaginatedResponse<ChatThreadDTO> | undefined) => {
                ...
                const updatedThread: ChatThreadDTO = index !== -1 
                    ? { ...items[index], body: update.body, timestamp: update.timestamp, unreadCount: update.direction === 'INBOUND' && normalizedUpdatePhone !== normalizedSelectedPhone ? (items[index].unreadCount || 0) + 1 : items[index].unreadCount }
                    : { phoneNumber: update.phoneNumber, body: update.body, timestamp: update.timestamp, unreadCount: update.direction === 'INBOUND' ? 1 : 0, username: null, isRegistered: false, isVerified: false };
            ...
            if (normalizedUpdatePhone === normalizedSelectedPhone) {
                ...
                queryClient.setQueriesData({ queryKey: ['thread', selectedPhone] }, (oldData: PaginatedResponse<MessageDTO> | undefined) => {
                    ...
                    const newMessage: MessageDTO = { id: `ws-temp-${Date.now()}`, whatsappId: `ws-${Date.now()}`, messageType: 'NOTIF', body: update.body, direction: update.direction, timestamp: update.timestamp, status: 'SENT', from: update.direction === 'INBOUND' ? update.phoneNumber : 'INTERNAL', to: update.direction === 'OUTBOUND' ? update.phoneNumber : 'INTERNAL' } as any;
    ...
    return (
        <motion.div 
            ...
            className="flex flex-col h-[calc(100vh-8rem)] bg-white rounded-2xl shadow-modern overflow-hidden border border-border text-left mx-auto w-full max-w-7xl"
        >
            <div className="flex h-full text-left overflow-hidden">
                {/* 01. SIDEBAR REGISTRY */}
                <div className={cn(
                    "w-full lg:w-[360px] xl:w-[420px] border-r border-border shrink-0 flex flex-col h-full bg-white",
                ...
                <div className={cn(
                    "flex-1 flex flex-col h-full bg-secondary/10",
