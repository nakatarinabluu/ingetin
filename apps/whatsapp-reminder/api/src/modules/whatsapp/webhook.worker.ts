...
import { MessageStatus, MessageDirection, Prisma } from '@prisma/client';
import { z } from 'zod';
...
const dlqQueue = new Queue('whatsapp-webhook-dlq', { connection: bullConnection });
                    ...
                    if (value.messages && Array.isArray(value.messages)) {
                        for (const msg of value.messages) {
                            ...
                            try {
                                ...
                                // --- SMART AI FINANCE PROCESSING ---
                                if (user) {
                                    const financeResult = await financeService.processWhatsAppMessage(user.id, msgBody);
                                    if (financeResult.success && financeResult.data) {
                                        // If AI successfully parsed a financial intent, reply to the user
                                        await whatsappService.sendText('notif', from, financeResult.data, user.id);
                                    }
                                }
                            } catch (err: unknown) {
