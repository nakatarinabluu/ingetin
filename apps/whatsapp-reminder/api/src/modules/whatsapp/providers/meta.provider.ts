...
export class MetaWhatsAppProvider implements MessagingProvider {
    ...
    private async callMetaApi(url: string, data: Record<string, unknown>, headers: Record<string, string>) {
    ...
    private async execute(config: { id: string, token: string }, data: Record<string, unknown>): Promise<MessagingResponse> {
        ...
        try {
            ...
            interface AxiosErrorShape { response?: { status?: number; data?: unknown } }
