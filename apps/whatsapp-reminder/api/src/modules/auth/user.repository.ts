...
export class UserRepository {
    ...
    async findProfile(id: string) {
        ...
        // Extract Google Refresh Token if it exists
        const googleAccount = profile.socialAccounts.find(a => a.provider === 'GOOGLE');
        const googleRefreshToken = googleAccount?.refreshToken || null;

        // Decrypt PII before returning
        const decrypted = this.encryptionService.decryptObject({
            ...profile,
            googleRefreshToken
        }, ['email', 'phoneNumber', 'googleRefreshToken']);
    ...
    async deleteSocialAccount(userId: string, provider: 'GOOGLE' | 'WHATSAPP' | 'DISCORD') {
        return await this.prismaDb.socialAccount.deleteMany({
