...
import { MessagingProvider } from '../whatsapp/providers/messaging.provider';
...
export class HealthController {
    ...
    getProviderHealth = async (req: FastifyRequest, reply: FastifyReply) => {
        ...
        try {
            ...
            logger.error({ msg: 'Health: Redis ping failed', error: (e as Error).message });
        ...
        const waProviderStatus = this.messagingProvider.getStatus ? this.messagingProvider.getStatus() : null;
        const googleProviderStatus = this.calendarService.getStatus();

        this.healthCache = {
            whatsapp: waProviderStatus ? (waProviderStatus.state === 'OPEN' ? 'DEGRADED' : 'OPERATIONAL') : (waStatus < 500 ? 'OPERATIONAL' : 'DEGRADED'),
            google: googleProviderStatus ? (googleProviderStatus.state === 'OPEN' ? 'DEGRADED' : 'OPERATIONAL') : (googleStatus === 200 ? 'OPERATIONAL' : 'DEGRADED'),
            database: dbStatus,
            redis: redisStatus
        };
        this.lastCheck = now;
