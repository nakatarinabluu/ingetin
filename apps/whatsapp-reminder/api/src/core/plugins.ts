...
export async function registerCorePlugins(fastify: FastifyInstance) {
    ...
    await fastify.register(helmet, {
        contentSecurityPolicy: {
            directives: {
                ...
                connectSrc: ["'self'", "ws:", "wss:", "https://*.ngrok.io", "https://*.ngrok-free.app", "http://localhost:*", "http://127.0.0.1:*"]
            }
        }
    });
    ...
    await fastify.register(cors, {
        origin: (origin, callback) => {
            ...
            // In dev, allow localhost. In prod, strictly follow allowedOrigins.
            const isLocalhost = origin.includes('localhost') || origin.includes('127.0.0.1');
            const isAllowed = allowedOrigins.some(o => origin.startsWith(o));

            if (isAllowed || (isDev && isLocalhost)) {
                callback(null, true);
            } else {
                logger.warn({ msg: 'CORS Denied', origin, allowedOrigins });
